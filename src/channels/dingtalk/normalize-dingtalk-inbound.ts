...
/**
 * 钉钉 Stream `/v1.0/im/bot/messages/get` → AgentMessageEnvelope；
 * 应跳过的消息 throw DingtalkInboundSkipError
 */
export function normalizeDingtalkInbound(
  ...
  if (message.robotCode) {
    channelMeta.robotCode = message.robotCode;
  }
  if (message.conversationType) {
    channelMeta.conversationType = message.conversationType;
  }
