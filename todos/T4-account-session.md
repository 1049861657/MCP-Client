...
| 配置 override：每账号覆盖层 + seed 回退 | 账号之间共享配置、超管管理 guest 默认配置 |
...
> 现网 `AIProvider`/`Setting`/`MCPServer`/`AgentProfile`/`RouteRule` 均为全局单份。本组改为 **两层 override**：`userId=null` 默认 seed（基线）+ 每账号覆盖行；生效配置 = 用户行优先、miss 回退 seed（8 个来源一致的市面标准：LibreChat cascade / GitLab cascading settings / ABP provider 链）。**默认 seed 只由 seed 脚本维护，不经任何 UI**。

- [ ] **T4-06-01** schema：配置表加 `userId?` 维度  
...
- [ ] **T4-06-02** 配置解析链：override 解析 + userId 透传  
  - 解析语义（GitLab 同款）：**用户行存在 → 用，不存在 → 回退 `userId=null` seed**；只存覆盖差异，不登录拷贝  
  - 标量配置（tool-prompt、参数）按键覆盖；集合配置（供应商、MCP 服务器、AgentProfile）：用户行归用户增删，seed 行只读可见、被同 key 用户行遮蔽  
  - `ConfigService` / `config-snapshot` / `profile-resolver` 入参增加 `userId?`；解析结果按 `(userId)` 键控 **TTL 缓存**（`lru-cache`，如 60s，LibreChat 同款；写操作即时失效）  
  - Envelope `channelMeta` 增加 `userId?`（Web authed 注入），Bus 链路透传给 Harness/配置解析  
  - 涉及：`src/services/config.service.ts`、`src/config-plane/*`、`channel.types.ts`、`normalize-web-inbound.ts`  
  - 验收：两账号各改各的互不可见；未覆盖项跟随 seed 更新自动生效；guest 始终默认值

- [ ] **T4-06-03** 写入语义与 UI 继承态  
...
- [ ] **T4-06-04** 供应商实例 per-user 化  
  - provider 实例/缓存按 `userId` 键控（guest/IM 用 `null` 键）；reload 只刷新自己的  
  - 涉及：`src/providers/ai-provider.ts` 加载链、`settings.controller.ts`  
  - 验收：账号 A 换 Key 不影响账号 B 与 guest 的调用

- [ ] **T4-06-05** MCP 连接按 `(userId, serverId)` 隔离  
