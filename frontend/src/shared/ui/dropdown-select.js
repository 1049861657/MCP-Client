 ...
 */
export function createDropdownSelect(config) {
  const select = document.createElement('select');
  select.className = ['field-select', config.className].filter(Boolean).join(' ');
  ...
  const variant = config.variant ?? 'group';
  if (variant === 'group' && config.groups?.length) {
  ...
  mountDropdownSelect(select, config);
  return select;
}
