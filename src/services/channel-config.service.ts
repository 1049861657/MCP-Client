...
export interface ChannelConfigMcpOption {
  id: string;
  name: string;
  /** 账号池中该服务的连接启用态（仅展示，Profile 选用由管理员勾选） */
  poolEnabled: boolean;
}
...
export interface ChannelConfigSaveResult {
  profile: ChannelConfigProfileValues;
  /** 用户点选但连通性探测未通过、未写入 Profile 的 MCP */
...
/** 读取渠道配置编辑器状态：Profile 已保存值 + 绑定账号资源池 + 账号默认值 */
...
/** 保存渠道 Profile 覆盖配置（选项须来自当前绑定账号资源池） */
export async function saveChannelConfig(options: {
  ...
  const effectiveVendor =
    input.vendor !== undefined ? input.vendor : existing.vendor;
